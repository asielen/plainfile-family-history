---
id: S-fb2345678c
title: Marriage certificate — Calvin Hartley & Harriet Webb, 1898
source_type: vital-record
source_date: 1898-04-22
source_class: original
repository: example collection
citation: >
  Breton County Clerk, Marriage Register Vol. 4, p. 112 (Calvin Hartley & Harriet Frances
  Webb, 22 April 1898, Fairview, Kansas). Fictional example record.
people: [P-fa7541e871, P-8h9jkambne]
files:
  - file: documents/vital-records/calvin-harriet-marriage-1898_S-fb2345678c.txt
    role: transcription
    status: fixture   # actual .txt created; fictional content
created: 2026-06-14
---

## Claims
```yaml
- value: "Calvin George Hartley married Harriet Frances Webb, 22 April 1898, Fairview, Kansas"
  id: C-fb0000001a
  type: marriage
  persons: [P-fa7541e871, P-8h9jkambne]
  roles:
    spouse: [P-fa7541e871, P-8h9jkambne]
  date: 1898-04-22
  place: L-7c1a9f4e22
  place_text: "Fairview, Breton County, Kansas"
  status: accepted
  reviewed: 2026-06-14
  confidence: high
  information: primary
  evidence: direct
  notes: Marriage register entry; date and place recorded by the county clerk.
```

## Notes
Fictional marriage certificate used to exercise vital-record source schema and to establish
the Calvin–Harriet union that underpins the Hartley–Bradford ancestor chain.
