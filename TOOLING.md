...
**Query.** `fha photoindex find --person P-… | --keyword … | --edtf 192X | --text "…"` → paths (filters AND together when combined). Default output dedupes matches to one row per group (`primary_path`); `--files` returns every matching raw row instead. Requires `.cache/photos.sqlite`: absent/unreadable is a clear error (exit 3); stale still queries but warns.
